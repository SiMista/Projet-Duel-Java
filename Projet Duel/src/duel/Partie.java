...
public class Partie {
	...
	public static boolean conditionPileAscendante(String entrée, int carte, Joueur j, Joueur jAdv) {
		if ((entrée.charAt(2) == '^') && (entrée.length() == 3)
				&& ((carte > j.getPileAscendante()) || (carte == j.getPileAscendante() - 10)))
			return true;
		else
			return false;
	}
	...
	public static boolean conditionPileDescendante(String entrée, int carte, Joueur j, Joueur jAdv) {
		if ((entrée.charAt(2) == 'v') && (entrée.length() == 3)
				&& ((carte < j.getPileDescendante()) || (carte == j.getPileDescendante() + 10)))
			return true;
		else
			return false;
	}
	...
	public static boolean vérifSaisie(String[] tab, Joueur j, Joueur jAdv) {
		...
		for (String entrée : tab) {
			...
			if (!Character.isDigit(entrée.charAt(0)) && !Character.isDigit(entrée.charAt(1))) // Vérifie si c'est un nombre
				erreur = false;
			if (entrée.length() > 4) // Vérifie que chaque 'entrée' ne fait pas plus de 4 caractères
				erreur = false;
			
			if (!((conditionPileAdverseAsc(entrée, carte, j, jAdv)) // Vérifie que l'entrée comporte les caractère des piles
